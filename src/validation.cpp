...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (fRequireStandard && !AreInputsStandard(tx, m_view, params.TaprootEnabled)) {
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/labyrinth-dev/2018-November/016518.html
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    // Start enforcing P2SH
    if (consensusparams.BIP16Enabled) {
        flags |= SCRIPT_VERIFY_P2SH;
    }

    // Start enforcing WITNESS if P2SH and Segwit are active
    if (consensusparams.BIP16Enabled && consensusparams.SegwitEnabled) {
        flags |= SCRIPT_VERIFY_WITNESS;
    }

    // Start enforcing BIP65 (CHECKLOCKTIMEVERIFY)
    if (consensusparams.BIP65Enabled) {
        flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
    }

    // Start enforcing BIP66 (DERSIG)
    if (consensusparams.BIP66Enabled) {
        flags |= SCRIPT_VERIFY_DERSIG;
    }

    // Start enforcing BIP112 (CHECKSEQUENCEVERIFY)
    if (consensusparams.CSVEnabled) {
        flags |= SCRIPT_VERIFY_CHECKSEQUENCEVERIFY;
    }

    // Start enforcing Taproot
    if (consensusparams.TaprootEnabled) {
        flags |= SCRIPT_VERIFY_TAPROOT;
    }

    // Start enforcing BIP147 (NULLDUMMY) if Segwit is active
    if (consensusparams.SegwitEnabled) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (chainparams.GetConsensus().CSVEnabled) {
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    if (fCheckPOW) {
        uint256 mix_hash;
        if (!CheckProofOfWork(block.GetHash(mix_hash), block.nBits, consensusParams))
...
void UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (commitpos != NO_WITNESS_COMMITMENT && consensusParams.SegwitEnabled && !block.vtx[0]->HasWitness()) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if((block.nVersion < 2 && nHeight >= consensusParams.BIP34Enabled) ||
       (block.nVersion < 3 && nHeight >= consensusParams.BIP66Enabled) ||
       (block.nVersion < 4 && nHeight >= consensusParams.BIP65Enabled))
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (consensusParams.BIP34Enabled)
    {
...
bool CChainState::RewindBlockIndex(const CChainParams& params)
{
    ...
    // blocks will be dealt with below (releasing cs_main in between).
    {
        LOCK(cs_main);
        for (const auto& entry : m_blockman.m_block_index) {
            if (params.GetConsensus().SegwitEnabled && !(entry.second->nStatus & BLOCK_OPT_WITNESS) && !m_chain.Contains(entry.second)) {
    ...
    int nHeight = 1;
    {
        ...
        while (nHeight <= m_chain.Height()) {
            ...
            if (params.GetConsensus().SegwitEnabled && !(m_chain[nHeight]->nStatus & BLOCK_OPT_WITNESS)) {
                break;
            }
            nHeight++;
        }
