...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (!State(nodeid)->fHaveWitness && consensusParams.SegwitEnabled) {
...
void PeerManager::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    bool fWitnessEnabled = m_chainparams.GetConsensus().SegwitEnabled;
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && ::ChainActive().Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !::ChainActive().Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        ...
                        (!m_chainparams.GetConsensus().SegwitEnabled || State(pfrom.GetId())->fHaveWitness)) {
    ...
    while (!orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(m_mempool, state, porphanTx, &removed_txn, false /* bypass_limits */)) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/labyrinth/labyrinth/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/labyrinth/labyrinth/pull/18044#discussion_r443419034
                                         ...
                                         const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (m_chainparams.GetConsensus().SegwitEnabled && !nodestate->fSupportsDesiredCmpctVersion) {
