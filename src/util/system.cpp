...
const char * const LABYRINTH_CONF_FILENAME = "labyrinth.conf";
const char * const LABYRINTH_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/labyrinth/labyrinth/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //labyrinth-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", LABYRINTH_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "labyrinth";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Labyrinth
    // macOS: ~/Library/Application Support/Labyrinth
    // Unix-like: ~/.labyrinth
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Labyrinth";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Labyrinth";
#else
    // Unix-like
    return pathRet / ".labyrinth";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LABYRINTH_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Labyrinth Core copyright is not removed by accident
    if (copyright_devs.find("Labyrinth Core") == std::string::npos) {
