    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return LabyrinthUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, LabyrinthUnits::SeparatorStyle::NEVER);
            else
                return LabyrinthUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+LabyrinthUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
