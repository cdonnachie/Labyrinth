...
#include <qt/labyrinthaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Labyrinth address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new LabyrinthAddressEntryValidator(parent));
    widget->setCheckValidator(new LabyrinthAddressCheckValidator(parent));
}

bool parseLabyrinthURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no labyrinth: URI
    if(!uri.isValid() || uri.scheme() != QString("labyrinth"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!LabyrinthUnits::parse(LabyrinthUnits::LAB, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseLabyrinthURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseLabyrinthURI(uriInstance, out);
}

QString formatLabyrinthURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("labyrinth:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(LabyrinthUnits::format(LabyrinthUnits::LAB, info.amount, false, LabyrinthUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openLabyrinthConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", LABYRINTH_CONF_FILENAME));
    ...
    /* Open labyrinth.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Labyrinth.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Labyrinth (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Labyrinth (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Labyrinth*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "labyrinth.desktop";
    return GetAutostartDir() / strprintf("labyrinth-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a labyrinth.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Labyrinth\n";
        else
            optionFile << strprintf("Name=Labyrinth (%s)\n", chain);
