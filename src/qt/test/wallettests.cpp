...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<LabyrinthAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_labyrinth-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_labyrinth-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_labyrinth-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = LabyrinthUnits::formatWithUnit(unit, balance, false, LabyrinthUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    LabyrinthAmountField* amountInput = receiveCoinsDialog.findChild<LabyrinthAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("labyrinth:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_labyrinth-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
