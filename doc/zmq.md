...
The Labyrinth Core daemon can be configured to act as a trusted "border
router", implementing the labyrinth wire protocol and relay, making
...
The ZeroMQ feature in Labyrinth Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building labyrinthd:
...
For instance:
    ...
    $ labyrinthd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/labyrinthd.tx.raw \
...
These options can also be provided in labyrinth.conf.
...
From the perspective of labyrinthd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into labyrinthd directly. Furthermore, no information is
...
using. Labyrinthd appends an up-counting sequence number to each
